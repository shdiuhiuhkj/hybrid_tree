...
path ='bupa'
path_result='./bupa/result/tree/'
path_result_dt='./bupa/result/tree/describe_tree/describe'
names =['Mcv', ' Alkphos', ' Sgpt', ' Sgot', ' Gammagt', ' Drinks', 'classes']
names_sort =['Mcv', ' Alkphos', ' Sgpt', ' Sgot', ' Gammagt', ' Drinks', 'classes']
names_nolabel =['Mcv', ' Alkphos', ' Sgpt', ' Sgot', ' Gammagt', ' Drinks']
numfeatures =['Mcv', ' Alkphos', ' Sgpt', ' Sgot', ' Gammagt', ' Drinks']
names_nolabel_nosort=['Mcv', ' Alkphos', ' Sgpt', ' Sgot', ' Gammagt', ' Drinks']
...
for b in range(5):
    df = pd.read_csv('./bupa/train_bupa_'+str(b)+'.csv', names=names, header=None)
    df_test = pd.read_csv('./bupa/test_bupa_'+str(b)+'.csv', names=names,
                          header=None)
    df_xy = df
    n = n + 1
    ...
    for random_num in ll:
        for j in [2]:
            ...
            sort_index = np.array(rf.sort_index)
            unique, count = np.unique(sort_index, return_counts=True)
            data_dict = dict(zip(unique, count))
            print(data_dict)
...
ccc_dtpd.to_csv(path_result_dt + 'clss_ratio_list_var3min_.csv')
...
ddd_dtpd.to_csv(path_result_dt + 'list_var3min_' + str(ll) + '_leaf2.csv')
...
dfdfa.to_csv(path_result + 'list_var3min_' + str(ll) + '.csv')
